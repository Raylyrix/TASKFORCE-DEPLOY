...
apiKeysRouter.get("/", requireUser, async (req, res, next) => {
  try {
...
// Get API key details
apiKeysRouter.get("/:id", requireUser, async (req, res, next) => {
  try {
    const userId = req.currentUser!.id;
...
// Create API key
apiKeysRouter.post("/", requireUser, async (req, res, next) => {
  try {
    const userId = req.currentUser!.id;
...
// Update API key
apiKeysRouter.put("/:id", requireUser, async (req, res, next) => {
  try {
    const userId = req.currentUser!.id;
    const apiKeyId = req.params.id;
...
// Revoke API key
apiKeysRouter.delete("/:id", requireUser, async (req, res, next) => {
  try {
    const userId = req.currentUser!.id;
    const apiKeyId = req.params.id;
