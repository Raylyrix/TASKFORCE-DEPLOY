...
gmailRouter.post("/labels", requireUser, async (req, res, next) => {
  try {
    const currentUser = req.currentUser;
    if (!currentUser) {
      res.status(401).json({ error: "Unauthorized" });
    ...
    const payload = createLabelSchema.parse(req.body ?? {});
    const client = await googleAuthService.getAuthorizedClientForUser(currentUser.id);
    const gmail = google.gmail({ version: "v1", auth: client });

    // Check if label already exists
    const { data: existingLabels } = await gmail.users.labels.list({ userId: "me" });
    const existing = existingLabels.labels?.find((l) => l.name === payload.name);
    ...
    if (existing) {
      ...
      return res.status(200).json({
        label: {
          id: existing.id ?? "",
          name: existing.name ?? "",
          type: (existing.type as "system" | "user") ?? "user",
          color: existing.color?.backgroundColor ?? null,
          messageListVisibility: existing.messageListVisibility ?? null,
          labelListVisibility: existing.labelListVisibility ?? null,
        },
        created: false,
      });
    }
    ...
    const { data: newLabel } = await gmail.users.labels.create({
      ...
      requestBody: {
        name: payload.name,
        messageListVisibility: payload.messageListVisibility ?? "show",
        labelListVisibility: payload.labelListVisibility ?? "labelShow",
        color: payload.color,
      },
    });
    ...
    res.status(201).json({
      label: {
        id: newLabel.id ?? "",
        name: newLabel.name ?? "",
        type: (newLabel.type as "system" | "user") ?? "user",
        color: newLabel.color?.backgroundColor ?? null,
        messageListVisibility: newLabel.messageListVisibility ?? null,
        labelListVisibility: newLabel.labelListVisibility ?? null,
      },
      created: true,
    });
  } catch (error) {
