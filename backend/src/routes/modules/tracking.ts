...
// Get daily analytics for dashboard (last 7 days)
trackingRouter.get("/daily-analytics", requireUser, async (req, res, next) => {
  try {
    const currentUser = req.currentUser;
    if (!currentUser) {
      res.status(401).json({ error: "Unauthorized" });
    ...
    // Get all tracking events for user's campaigns in the last N days
    ...
    // Group by date
    const dailyData = new Map<string, { opens: number; clicks: number }>();
    ...
    for (let i = 0; i < days; i++) {
      ...
      dailyData.set(dateKey, { opens: 0, clicks: 0 });
    }
    ...
    const result = Array.from(dailyData.entries()).map(([dateKey, data]) => {
    ...
    res.status(200).json({ dailyData: result });
  } catch (error) {
    next(error);
  }
});
